...
def checkpoint(epoch,loss,psnr,ssim):
    model.eval()
    if use_gpu:
...
def train(epoch):
    ...
    for iteration, sample in enumerate(dataloader):#difference between (dataloader) &(dataloader,1)
        ...
        if iteration%100==0:
            print("===> Epoch[{}]({}/{}):loss: {:.4f}".format(epoch, iteration, len(dataloader), loss.data[0]))  
...
def main():
    ...
    for epoch in range(args.epochs):
        loss=train(epoch)
        psnr,ssim = test()
        checkpoint(epoch,loss,psnr,ssim)
...
model=mymodel.MemNet(3,64,6,6)#(self, in_channels, channels, num_memblock, num_resblock)
